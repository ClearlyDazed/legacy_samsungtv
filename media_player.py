"""Support for interface with a legacy Samsung TV."""
...
SOURCES = {'HDMI 1': ['KEY_TV', 'KEY_SOURCE', 'KEY_RIGHT', 'KEY_ENTER'], 'HDMI 2': ['KEY_TV', 'KEY_SOURCE', 'KEY_RIGHT', 'KEY_RIGHT', 'KEY_ENTER']}
...
async def async_setup_entry(hass, config_entry, async_add_entities):
    """Set up the legacy Samsung TV from a config entry."""
...
class SamsungTVDevice(MediaPlayerEntity):
    """Representation of a legacy Samsung TV."""
    ...
    def send_keylist(self, key):
        """Send a key from source list to the tv and handles exceptions."""
        if self._power_off_in_progress() and key != "KEY_POWEROFF":
            LOGGER.info("TV is powering off, not sending command: %s", key)
            return
        self._bridge.send_keylist(key)
    ...
    def select_source(self, source):
        ...
        self.send_keylist(SOURCES[source])
