"""samsungTVlegacy and samsungtvws bridge classes."""

from abc import ABC, abstractmethod
from samsungTVlegacy import Remote
from samsungTVlegacy.exceptions import AccessDenied, ConnectionClosed, UnhandledResponse
...
class SamsungTVBridge(ABC):
    ...
    def send_keylist(self, key_list):
        """Send the keys from source list to the tv and handles exceptions."""
        try:
            # recreate connection if connection was dead
            retry_count = 1
            for _ in range(retry_count + 1):
                try:
                    self._send_keylist(key_list)
                    break
                except (
                    ConnectionClosed,
                    BrokenPipeError,
    ...
    @abstractmethod
    def _send_keylist(self, key_list):
        """Send the keys."""
...
class SamsungTVLegacyBridge(SamsungTVBridge):
    ...
    def _send_keylist(self, key_list):
        """Send the key from source list using legacy protocol."""
        for key in key_list:
            self._get_remote().control(key)
...
class SamsungTVWSBridge(SamsungTVBridge):
    ...
    def _send_keylist(self, key_list):
        """Send the key from source list using websocket protocol."""
        for key in key_list:
            self._get_remote().send_key(key)
