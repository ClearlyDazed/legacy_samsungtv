"""BRIDGE for Legacy Samsung TV and samsungtvws bridge classes."""
...
async def async_get_device_info(
    ...
    for port in WEBSOCKET_PORTS:
        ...
        if info := await bridge.async_device_info():
            LOGGER.debug(
                "Fetching rest info via %s was successful: %s, ",
                port,
                info,
            )
...
class SamsungTVWSBaseBridge[
    _RemoteT: (SamsungTVWSAsyncRemote),
    _CommandT: (SamsungTVCommand),
](SamsungTVBridge):
